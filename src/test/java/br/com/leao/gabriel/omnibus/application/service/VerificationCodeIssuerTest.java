...
class VerificationCodeIssuerTest {
  ...
  @Mock
  private TokenIssuanceRateLimiterPort rateLimiter;
  ...
  void shouldIssueCode() {
    when(rateLimiter.incrementAndGet(USER_ID, TokenType.ACCOUNT_ACTIVATION))
        .thenReturn(1L);
    ...
    verify(rateLimiter)
        .incrementAndGet(USER_ID, TokenType.ACCOUNT_ACTIVATION);
  ...
  void shouldAllowThreeTokens() {
    ...
    assertThat(issuer.issue(USER_ID, TokenType.ACCOUNT_ACTIVATION))
        .isEqualTo(CODE);
  ...
  void shouldPersistHashedCode() {
    when(rateLimiter.incrementAndGet(USER_ID, TokenType.PASSWORD_RESET))
        .thenReturn(1L);
    when(otpGenerator.generateCode()).thenReturn(CODE);
    when(otpGenerator.hash(CODE)).thenReturn(HASH);
    ...
    ArgumentCaptor<UserToken> captor =
        ArgumentCaptor.forClass(UserToken.class);
  ...
  void shouldUseDefaultExpiration() {
    ...
    assertThat(savedToken.getExpiresAt())
        .isBetween(before.minusSeconds(2), after.plusSeconds(2));
  }
