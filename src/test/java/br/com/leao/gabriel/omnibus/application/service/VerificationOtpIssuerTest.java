...
class VerificationOtpIssuerTest {
  ...
  @Mock
  private OtpIssuanceRateLimiterPort rateLimiter;
  ...
  void shouldIssueCode() {
    ...
    verify(userTokenRepository).revokeActiveTokens(USER_ID);
  }
  ...
  void shouldIssueEmailChangeToken() {
    ...
    String result = issuer.issueForEmailChange(USER_ID, "new@example.com");
    ...
    assertThat(savedToken.getTargetEmail()).isEqualTo("new@example.com");
    assertThat(savedToken.getCodeHash()).isEqualTo(HASH);
    verify(userTokenRepository).revokeActiveTokens(USER_ID);
  }
